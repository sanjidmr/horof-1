...
import { 
  ...
  Phone, User, Calendar, CreditCard, Loader2, Home, ShieldCheck
} from 'lucide-react';
...
function PaymentSuccessContent() {
    ...
    return (
      <div className="min-h-screen bg-slate-50 flex flex-col items-center justify-center p-6">
        <div className="flex flex-col items-center gap-3">
          ...
          <p className="text-sm font-bold text-slate-600">Retrieving payment status...</p>
        </div>
      </div>
    );
  }
      ...
      <div className="min-h-screen bg-slate-50 flex flex-col items-center justify-center p-6 text-center">
        <div className="max-w-md bg-white p-8 rounded-3xl border border-slate-100 shadow-xl space-y-6">
          ...
          <div>
            ...
            <p className="text-sm text-slate-500 mt-2">We could not confirm payment details for order ID #{orderId}. Please contact support if you believe this is an error.</p>
          </div>
  ...
  const totalAmount = Number(order.total ?? order.amount ?? (subtotal + deliveryCharge));
    ...
    <div className="min-h-screen bg-slate-50/50 py-12 px-4 sm:px-6 lg:px-8">
      <div className="max-w-3xl mx-auto space-y-8">
        ...
        <div className="bg-white rounded-3xl border border-[#1b4332]/10 shadow-sm overflow-hidden relative">
          <div className="absolute top-0 left-0 w-full h-2 bg-gradient-to-r from-emerald-600 to-[#1B4332]" />
          
          <div className="p-8 text-center space-y-4">
            <div className="h-16 w-16 bg-emerald-50 text-emerald-600 rounded-full flex items-center justify-center mx-auto border border-emerald-100/50 animate-pulse">
              <ShieldCheck className="w-10 h-10 text-emerald-600" />
            </div>
            <div>
              <h1 className="text-3xl font-display font-bold text-slate-900">Payment Successful!</h1>
              <p className="text-sm text-slate-500 mt-1">Your payment was successfully validated by the secure gateway. Thank you!</p>
            </div>
            <div className="flex flex-wrap items-center justify-center gap-2.5">
              <span className="inline-flex items-center gap-1.5 px-3 py-1 bg-slate-50 border border-slate-200/50 rounded-full text-xs font-mono font-bold text-slate-600">
                Order ID: #{String(order.id).toUpperCase()}
              </span>
              {order.transaction_id && (
                <span className="inline-flex items-center gap-1.5 px-3 py-1 bg-blue-50 border border-blue-200/50 rounded-full text-xs font-mono font-bold text-blue-600">
          ...
          <div className="bg-white rounded-3xl border border-slate-100 shadow-sm p-6 space-y-6">
            ...
            <div className="border-t border-slate-100 pt-4 space-y-2 text-sm font-semibold">
              ...
              <div className="flex justify-between font-bold text-slate-900 pt-3 border-t border-slate-100 text-base">
                ...
                <span className="text-[#1B4332]">৳{totalAmount.toLocaleString()}</span>
              </div>
            </div>
          </div>
            ...
            <div className="space-y-6">
              ...
              <div className="space-y-4 text-sm font-medium text-slate-700">
                <div className="flex gap-3">
                  ...
                  <div>
                    <p className="text-xs text-slate-400 font-bold uppercase tracking-wider">Customer Name</p>
                ...
                <div className="flex gap-3">
                  ...
                  <div>
                    ...
                    <p className="text-[#1B4332] font-bold uppercase mt-0.5">Online Payment (Verified)</p>
                  </div>
                </div>
              </div>
            </div>
