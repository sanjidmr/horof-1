...
import {
  ...
  warehouseMarkOutForDelivery,
  warehouseMarkDelivered,
  warehouseMarkReturned,
...
function shortOrderRef(assignment: WarehouseOrder): string {
...
const STATUS_META: Record<string, { label: string; color: string; bg: string; icon: any }> = {
  ...
  out_for_delivery: { label: 'Out for Delivery', color: 'text-indigo-700', bg: 'bg-indigo-100', icon: Send },
  delivered: { label: 'Delivered', color: 'text-emerald-700', bg: 'bg-emerald-100', icon: CheckCheck },
...
export default function WarehouseOrdersPage() {
  ...
  const fetchOrders = useCallback(async () => {
    ...
    const admin = isInternalAdminRole(profile?.role);
    setWarehouseId(whId);
    setIsAdmin(admin);
    ...
    try {
      if (admin && !whId) {
        // Admin view: load EXCLUSIVELY from warehouse_assignments (joined
        // to orders/warehouses), never from orders directly — assignments
  ...
  const handleAction = async (order: WarehouseOrder, action: string) => {
    if (order.synthetic) {
      toast.error('This order has no warehouse assignment yet');
      return;
    }
    ...
    try {
      switch (action) {
        ...
        case 'out_for_delivery':
          await warehouseMarkOutForDelivery(order.id, editNotes || null);
          toast.success('Marked as out for delivery!');
          break;
        case 'delivered':
          await warehouseMarkDelivered(order.id, editNotes || null);
        ...
        case 'returned':
          ...
          await warehouseMarkReturned(order.id, editNotes);
    ...
    <div className="space-y-6 pb-10">
      ...
      <div className="grid grid-cols-2 lg:grid-cols-5 gap-3">
        {[
          ...
          { label: 'Out for Delivery', value: stats.outForDelivery, color: 'text-indigo-600', bg: 'bg-indigo-100', icon: Send },
          { label: 'Delivered', value: stats.delivered, color: 'text-emerald-600', bg: 'bg-emerald-100', icon: CheckCheck },
          { label: 'Returned', value: stats.returned, color: 'text-orange-600', bg: 'bg-orange-100', icon: RotateCcw },
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/60 backdrop-blur-sm p-4" onClick={() => setSelected(null)}>
          <div className="bg-white rounded-3xl w-full max-w-4xl shadow-2xl max-h-[94vh] flex flex-col overflow-hidden" onClick={(e) => e.stopPropagation()}>
            ...
            <div className="overflow-y-auto flex-1 p-6 space-y-5">
              ...
              {/* Read Only (no warehouse assignment) */}
              {selected.synthetic && (
                <section className="bg-amber-50 rounded-2xl p-4 border border-amber-100">
                  <h3 className="text-xs font-bold text-amber-700 uppercase tracking-widest mb-2 flex items-center gap-2">
                    <AlertTriangle className="h-3.5 w-3.5" /> Read Only
                  </h3>
                  <p className="text-sm text-amber-700">This order has no warehouse assignment yet, so warehouse actions and packing files are disabled. Assign it to a warehouse to start processing.</p>
                </section>
              )}
            ...
            <div className="p-5 border-t border-slate-100 space-y-3 bg-slate-50/50">
              ...
              {selected.status === 'ready_for_dispatch' && (
                <>
                  <button
                    disabled={!!actionLoading}
                    onClick={() => handleAction(selected, 'out_for_delivery')}
                    className="w-full h-11 rounded-xl bg-indigo-600 hover:bg-indigo-700 text-white font-bold text-sm flex items-center justify-center gap-2"
                  >
                    {actionLoading === selected.id + '_out_for_delivery' ? <Loader2 className="h-4 w-4 animate-spin" /> : <Send className="h-4 w-4" />}
                    Out for Delivery
                  </button>
                  <button
                    disabled={!!actionLoading}
              ...
              {selected.status === 'out_for_delivery' && (
                <>
                  <button
                    disabled={!!actionLoading}
                    onClick={() => handleAction(selected, 'delivered')}
                    className="w-full h-11 rounded-xl bg-emerald-600 hover:bg-emerald-700 text-white font-bold text-sm flex items-center justify-center gap-2"
                  >
                    {actionLoading === selected.id + '_delivered' ? <Loader2 className="h-4 w-4 animate-spin" /> : <CheckCheck className="h-4 w-4" />}
                    Mark as Delivered
                  </button>
                  <button
                    disabled={!!actionLoading}
                    onClick={() => handleAction(selected, 'returned')}
                    className="w-full h-11 rounded-xl border-2 border-orange-200 bg-white text-orange-600 hover:bg-orange-50 font-bold text-sm flex items-center justify-center gap-2"
                  >
