...
export default function WarehouseProductsPage() {
  const { user, isWarehouseStaff, userRole } = useAuth();
  ...
  const fetchProfile = useCallback(async () => {
    ...
    const { data: profile } = await sb
      ...
      .eq('id', u.id).single();
    if (profile?.assigned_warehouse_id) setWarehouseId(profile.assigned_warehouse_id);
  }, []);
  ...
  const filtered = products
    .filter(p => {
      ...
      if (stockFilter === 'assigned' && p.default_warehouse_id !== warehouseId) return false;
      if (stockFilter === 'unassigned' && p.default_warehouse_id !== null) return false;
      return true;
    })
    .sort((a, b) => {
  ...
  const stats = {
    ...
    unassigned: products.filter(p => !p.default_warehouse_id).length,
    ...
    <div className="space-y-6">
      ...
      <div className="grid grid-cols-2 sm:grid-cols-5 gap-3">
        {[
          ...
          { label: 'Unassigned', value: stats.unassigned, icon: AlertTriangle, color: 'text-amber-600' },
      ...
      <div className="bg-white rounded-2xl border border-slate-100 p-4 flex flex-wrap gap-3 items-center">
        ...
        >
          ...
          <option value="assigned">Assigned to My Warehouse</option>
