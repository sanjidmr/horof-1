...
import { BarTrendChart } from '@/components/admin/reports/LazyCharts';
      ...
      <div className="grid gap-6 lg:grid-cols-2">
        <ReportCard title="Top Products by Revenue" subtitle="Best performers in this period">
          {data.performance.length === 0 ? (
            ...
            <BarTrendChart
              data={data.performance.slice(0, 10).map((p) => ({ label: p.name.length > 22 ? `${p.name.slice(0, 22)}â€¦` : p.name, revenue: p.revenue }))}
              xKey="label"
