...
import { BarTrendChart, ComposedTrendChart, DonutChart, ChartLegend } from '@/components/admin/reports/LazyCharts';
...
export function ProfitLossClient() {
  ...
  const linesColumns: ReportColumn<any>[] = [
    ...
    { key: 'amount', label: 'Amount', align: 'right', render: (r) => <span className={r.kind === 'revenue' ? 'font-bold text-emerald-600' : r.kind === 'profit' ? 'font-bold text-slate-900' : 'text-red-500'}>{r.kind === 'expense' ? 'âˆ’' : '+'}{formatCurrency(r.amount)}</span> },
  ];
