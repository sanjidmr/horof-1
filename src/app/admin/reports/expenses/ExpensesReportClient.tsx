...
import { AreaTrendChart, BarTrendChart, DonutChart, ChartLegend } from '@/components/admin/reports/charts';
...
export function ExpensesReportClient() {
  ...
  const recentColumns: ReportColumn<any>[] = [
    ...
    { key: 'category', label: 'Category' },
    { key: 'notes', label: 'Notes', render: (r) => r.notes || r.description || '—' },
    { key: 'paymentMethod', label: 'Paid Via', render: (r) => titleCase(r.paymentMethod) },
    { key: 'amount', label: 'Amount', align: 'right', render: (r) => <span className="font-bold text-red-500">−{formatCurrency(r.amount)}</span> },
  ];
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="grid grid-cols-2 gap-4 md:grid-cols-4">
        ...
        <StatCard label="Top Category" value={k.topCategory ? titleCase(k.topCategory) : '—'} icon={<Trophy className="h-5 w-5" />} accent="violet" />
      </div>
