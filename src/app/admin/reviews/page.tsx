...
import { adminGetAllReviews, adminApproveReview, adminRejectReview, adminDeleteReview, adminReplyToReview } from '@/lib/actions/reviews';
...
export default function AdminReviewsPage() {
  ...
  async function fetchReviews() {
    setLoading(true);
    try {
      const allReviews = await adminGetAllReviews({}) as AdminReview[];
      ...
      console.error('Error fetching reviews:', err?.message || String(err));
  ...
  async function handleApprove(reviewId: string) {
    startTransition(async () => {
      const result = await adminApproveReview(reviewId);
      if (!result.success) {
  ...
  async function handleReject(reviewId: string) {
    startTransition(async () => {
      const result = await adminRejectReview(reviewId);
      if (!result.success) {
  ...
  async function handleReply(reviewId: string) {
    ...
    startTransition(async () => {
      const result = await adminReplyToReview(reviewId, replyText);
      if (!result.success) {
