...
export default function TrackOrderPage() {
  ...
  const handleReturnClick = async () => {
    ...
    try {
      const res = await requestOrderReturnAction(String(order.id), returnReason, returnDetails);
      if (res.success) {
            ...
            <div className="bg-white rounded-[2.5rem] p-8 max-w-md w-full border border-slate-100 shadow-2xl space-y-6">
              ...
              <div className="space-y-2">
                ...
                <select
                  ...
                  className="w-full p-3 bg-slate-50 border border-slate-100 rounded-2xl text-xs text-slate-900 outline-none focus:bg-white focus:border-[#1a4731] transition-all cursor-pointer"
                  required
                >
              ...
              <div className="flex gap-3 justify-end">
                <button 
                  onClick={() => { setReturnOpen(false); setReturnReason(''); setReturnDetails(''); }}
                  className="h-12 px-6 rounded-2xl border text-xs font-bold hover:bg-slate-50 cursor-pointer"
                >
                  Close
                </button>
