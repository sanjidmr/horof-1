...
import { submitContactMessage, uploadContactFile, type ContactAttachment } from '@/lib/actions/contact';
...
export default function ContactPage() {
  ...
  // Upload the file (if any) and return a structured attachment object.
  const uploadAttachment = async (file: File | null): Promise<ContactAttachment | null> => {
    if (!file) return null;
    const fd = new FormData();
    fd.append('file', file);
    const uploadResult = await uploadContactFile(fd);
    if (uploadResult?.error || !uploadResult?.url) {
      toast.error(uploadResult?.error || 'Failed to upload file. Please try again.');
      return null;
    }
    return {
  ...
  const submitForm = async (data: {
    ...
    try {
      // Reconstruct the File from base64 if we came back from the auth
      // redirect (File objects don't survive JSON serialization).
      let fileObj: File | null = data.fileObj || null;
      if (!fileObj && data.fileBase64 && data.fileMeta) {
        const byteChars = atob(data.fileBase64);
        const byteNumbers = new Array(byteChars.length);
        for (let i = 0; i < byteChars.length; i++) byteNumbers[i] = byteChars.charCodeAt(i);
        const byteArray = new Uint8Array(byteNumbers);
        fileObj = new File([byteArray], data.fileMeta.name, {
          type: data.fileMeta.type,
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    // Serialize the file as base64 so it survives the auth redirect
    // (File objects become {} when JSON.stringify'd).
    let fileBase64 = '';
    let fileMeta: { name: string; size: number; type: string } | undefined;
    if (uploadedFile) {
      fileBase64 = await readFileAsBase64(uploadedFile);
      fileMeta = { name: uploadedFile.name, size: uploadedFile.size, type: uploadedFile.type };
    }

    const formData = {
      name, email, phone, inquiryType, message,
      fileObj: uploadedFile || undefined,
      fileBase64,
      fileMeta,
    };
              ...
              <div className="space-y-1.5">
                ...
                {!uploadedFile ? (
                  ...
                  >
                    ...
                    <span className="text-[10px]">PNG, JPG, PDF, DOCX, DOC, ZIP, GIF supported</span>
                  </button>
                ) : (
                ...
                <input
                  ...
                  accept=".png,.jpg,.jpeg,.gif,.pdf,.docx,.doc,.zip"
