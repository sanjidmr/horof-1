...
function ProductsPageContent() {
  ...
  const subcategoriesByCategory: Record<string, any[]> = useMemo(() => {
  ...
  useEffect(() => {
    async function fetchData() {
      ...
      const [prodRes, catRes, subRes] = await Promise.all([
        supabase
          .from('products')
          .select('id,name,slug,description,price,offer_price,stock,is_active,created_at,section,product_images(url,sort_order),categories(name),reviews(rating)')
          .eq('is_active', true)
  ...
  const filteredProducts = useMemo(() => dbProducts
    .filter(p => {
    ...
    [dbProducts, selectedCategory, deferredSearch, priceRange, sortBy]);
