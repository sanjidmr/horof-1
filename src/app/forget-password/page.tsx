...
export default function ForgotPasswordPage() {
  ...
  return (
    <div className="min-h-screen pt-24 md:pt-32 pb-16 md:pb-24 px-4 sm:px-6 flex items-center justify-center relative overflow-hidden bg-bg-primary">
      <motion.div
        ...
        <form className="bg-bg-card border border-border-forest rounded-3xl md:rounded-[40px] p-6 md:p-10 space-y-5 md:space-y-6 shadow-xl shadow-accent-primary/5 glass-card">
          {!isSent ? (
            ...
            <div className="text-center space-y-4 py-2 md:py-4">
              ...
              <Button variant="secondary" className="w-full h-11 md:h-12 rounded-xl text-[10px] md:text-xs font-bold tracking-widest uppercase border-border-forest" onClick={() => setIsSent(false)}>
                Resend Link
              </Button>
            </div>
          )}
