...
import { 
  ...
  MapPin, User, ShieldCheck, Calendar,
  CheckCircle2, ShoppingCart, X, AlertTriangle, History
} from 'lucide-react';
  ...
  const fetchData = async () => {
    try {
      ...
      try {
        ...
        if (stored) {
          const ids = safeJsonParseArray<string>(stored);
          if (ids.length > 0) {
      ...
      <div className="relative rounded-3xl overflow-hidden shadow-xl border border-[#1b4332]/5 bg-gradient-to-tr from-[#1B4332] via-[#24543d] to-[#2D6A4F] p-6 md:p-10 text-white">
        ...
        <div className="flex flex-col lg:flex-row lg:items-center justify-between gap-8 relative z-10">
          ...
          <div className="flex flex-wrap justify-center lg:justify-end gap-3.5 border-t border-white/15 lg:border-none pt-6 lg:pt-0">
            <Link href="/customer/profile" className="px-5 py-3 bg-white text-[#1B4332] hover:bg-slate-100 font-bold rounded-2xl text-xs uppercase tracking-wider transition-all duration-300 shadow-md flex items-center gap-2">
