...
// Authenticate the actor via strict RBAC and return a service-role client
// (bypasses RLS). Admin review management reads/writes ALL reviews, so it
// must not depend on per-table RLS policies that may not cover every internal
// role. Access is granted exclusively through the reviews.* permissions.
async function requireReviewAdmin(permission: string): Promise<SupabaseClient> {
...
export async function adminGetAllReviews(filter: AdminReviewFilter = {}): Promise<ReviewWithProfile[]> {
  ...
  try {
    admin = await requireReviewAdmin('reviews.view');
  } catch (e: any) {
    console.error('[adminGetAllReviews]', e?.message);
    return [];
  }
...
export async function adminApproveReview(reviewId: string): Promise<{ success: boolean; error?: string }> {
  let admin: SupabaseClient;
  try {
    admin = await requireReviewAdmin('reviews.edit');
  } catch (e: any) {
...
export async function adminGetReviewCounts(): Promise<{
  ...
  const admin = createSupabaseAdminClient();
  if (!admin) return { total: 0, approved: 0, pending: 0 };

  const [totalRes, approvedRes, pendingRes] = await Promise.all([
    admin.from('product_reviews').select('id', { count: 'exact', head: true }),
    admin.from('product_reviews').select('id', { count: 'exact', head: true }).eq('is_approved', true),
    admin.from('product_reviews').select('id', { count: 'exact', head: true }).eq('is_approved', false),
  ]);
