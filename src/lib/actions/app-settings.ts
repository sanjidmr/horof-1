...
async function requireAdmin() {
  ...
  // STRICT RBAC — the permission matrix is the single source of truth.
  // The settings module requires the `settings_center.manage` permission
  // (granted to super_admin/owner and any role explicitly assigned it).
  // The legacy profiles.role ('admin') check is intentionally NOT used —
  // it would bypass the granular permission system.
 ...
 */
// Simple in-memory cache for public settings to avoid repeated DB reads during a single dev session
// The cache stores the RESOLVED public settings object (not the Promise);
// `any` is used intentionally to avoid a circular type reference between the
// cache variable and the function it annotates.
let cachedPublicSettings: { ts: number; value: any } | null = null;
let pendingPublicSettings: Promise<any> | null = null;
const PUBLIC_SETTINGS_TTL = 30 * 1000; // 30 seconds

export async function getPublicSettings() {
  // Return cached value when available and fresh
  if (cachedPublicSettings && Date.now() - cachedPublicSettings.ts < PUBLIC_SETTINGS_TTL) {
    return cachedPublicSettings.value as any;
  }

  // Single-flight: concurrent callers (e.g. generateMetadata + RootLayout)
  // share ONE database read instead of each issuing its own query.
  if (!pendingPublicSettings) {
    pendingPublicSettings = (async () => {
      const supabase = await createSupabaseServerClient();
      if (!supabase) {
        const fallback = {
          general: { ...DEFAULT_GENERAL },
        ...
        cachedPublicSettings = { ts: Date.now(), value: fallback as any };
        return fallback;
      }
      ...
      cachedPublicSettings = { ts: Date.now(), value: result as any };
      return result;
    })().finally(() => {
      pendingPublicSettings = null;
    });
  }

  return pendingPublicSettings as any;
}
...
export async function changeSuperAdminPassword(input: {
  ...
  // STRICT RBAC: password change requires the security_center.manage
  // permission (super_admin/owner or an explicitly granted role).
  const { requirePermission } = await import('./security');
  try {
