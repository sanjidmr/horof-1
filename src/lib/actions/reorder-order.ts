...
export async function reorderOrder(originalOrderId: string) {
  ...
  const { data: orderRequest, error: reqErr } = await supabase
    ...
    .insert({
      ...
      coupon_code: null,
      design_charge: 0,
      customer_notes: `Reorder of Order #${originalOrder.order_number || originalOrderId}`,
  ...
  try {
    await createNotification({
      ...
      message: `Reorder from ${customerName} — original order #${originalOrder.order_number || originalOrderId}`,
      type: 'order',
    });
  } catch {}
