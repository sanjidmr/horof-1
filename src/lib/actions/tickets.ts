...
export async function addTicketReply(ticketId: string, message: string, isInternalNote: boolean = false) {
  ...
  const isInternal = profile?.user_type === 'internal' || isInternalAdminRole(profile?.role);
  const senderRole = isInternal ? 'admin' : 'customer';
  ...
  const { error } = await supabase
    ...
    .insert({
      ...
      message,
      is_internal_note: isInternal && isInternalNote,
    });
