...
function buildPayload(d: ProductFormParsed) {
  ...
  // Build perfect_for as text[] array (matches DB column type: text[])
  // Try perfect_for_tags first (tag-based UI), fall back to perfect_for_str (comma-separated)
  const perfectForArray: string[] = (() => {
    // Check if we have array-based tags
    if (Array.isArray(d.perfect_for_tags) && d.perfect_for_tags.length > 0) {
      return d.perfect_for_tags.map((s: string) => s.trim()).filter(Boolean);
    }
    // Fall back to comma-separated string
    if (d.perfect_for_str && d.perfect_for_str.trim()) {
      return d.perfect_for_str
        .split(',')
        .map((s) => s.trim())
        .filter(Boolean);
    }
  ...
  // Ensure we always have a non-null array (PostgreSQL text[] column)
  const perfectForPayload: string[] = perfectForArray.length > 0 ? perfectForArray : [];
  ...
  return {
    ...
    perfect_for: perfectForPayload,
    // Keep the JSONB column in sync for backward compatibility with the migration
    // that added perfect_for_tags (20260720000005). Some consumers may read it.
    perfect_for_tags: perfectForPayload,
    section: section,
    ...
    default_warehouse_id: (d as any).default_warehouse_id ?? null,
    order_config: {
