      ...
      <div className="flex flex-col lg:flex-row items-center justify-between gap-6 lg:gap-12 relative z-10">
        <div className="max-w-xl space-y-4 md:space-y-6 text-center lg:text-left">
          ...
          <h2 className="text-2xl sm:text-3xl md:text-5xl font-display font-medium text-white leading-tight">
            {subscribed ? 'You\'re in!' : 'Join the <span className="text-accent-light italic">Horof</span>  group'}
          </h2>
          {subscribed ? (
            <p className="text-white/80 text-sm flex items-center gap-2">
              <CheckCircle className="w-5 h-5 text-emerald-400" /> Thank you for subscribing. Check your inbox for updates.
            </p>
          ) : (
        ...
        {!subscribed && (
          <div className="w-full lg:max-w-md">
            <form onSubmit={handleSubmit} className="flex flex-col sm:flex-row gap-2 sm:gap-3 p-1.5 sm:p-2 bg-white/5 backdrop-blur-xl rounded-xl border border-white/10 focus-within:border-accent-light/50 transition-all shadow-inner">
              <input
                type="email"
                value={email}
                ...
                required
              />
              <Button type="submit" variant="primary" disabled={loading} className="px-6 sm:px-8 bg-accent-light hover:bg-white text-accent-primary rounded-lg h-10 sm:h-12 transition-all font-bold uppercase tracking-widest text-[9px] sm:text-[10px]">
                {loading ? <Loader2 className="w-4 h-4 animate-spin" /> : <>Subscribe <ArrowRight className="ml-2 h-3 w-3" /></>}
              </Button>
            </form>
